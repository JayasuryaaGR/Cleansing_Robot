...
def camera_top(msg):
    ...
    if cam:
        global y_min_blue, y_min_green, y_min_red, y_max_blue, y_max_green, y_max_red, yta
        global r_min_blue, r_min_green, r_min_red, r_max_blue, r_max_green, r_max_red, rta
        global p_min_blue, p_min_green, p_min_red, p_max_blue, p_max_green, p_max_red, pta
        global g_min_blue, g_min_green, g_min_red, g_max_blue, g_max_green, g_max_red, gta
        global panda, pub_move, move, detectedt, bridget, r_detectedt, p_detectedt, g_detectedt, y_detectedt
        global then, now, robot_ini

        # print("From Cam Top")
        now = time.time()

        image = bridget.imgmsg_to_cv2(msg, "bgr8")
        image = image[:,560:640]
        # cv2.imshow("Image window", image)
        image_HSV = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
        # cv2.imshow("Image window", image_HSV)
        ...
        if y_contours and not detectedt and not y_detectedt:
            if cv2.contourArea(y_contours[0]) > yta:
                then = time.time()
                now = time.time()
                detectedt, y_detectedt = True, True
                print("From Top YD")
                move.angular.z = 0
                move.linear.x = 0
                pub_move.publish(move)
        ...
        elif r_contours and not detectedt and not r_detectedt:
            if cv2.contourArea(r_contours[0]) > rta:
                then = time.time()
                now = time.time()
                detectedt, r_detectedt = True, True
                print("From Top RD")
                move.angular.z = 0
                move.linear.x = 0
                pub_move.publish(move)
        ...
        # elif p_contours and not detectedt and not p_detectedt:
        #     if cv2.contourArea(p_contours[0]) > pta and cv2.contourArea(p_contours[0]) < 1000:
        #         then = time.time()
        #         now = time.time()
        #         detectedt, p_detectedt = True, True
        #         print("From Top PD")
        #         move.angular.z = 0
        #         move.linear.x = 0
        #         pub_move.publish(move)
        ...
        elif g_contours and not detectedt and not g_detectedt:
            if cv2.contourArea(g_contours[0]) > gta:
                then = time.time()
                now = time.time()
                detectedt, g_detectedt = True, True
                print("From Top GD")
                move.angular.z = 0
                move.linear.x = 0
                pub_move.publish(move)
...
def camera_bottom(msg):
    ...
    if cam:
        ...
        #     if cv2.contourArea(p_contours[0]) > pba and cv2.contourArea(p_contours[0]) < 55000:
        #         then = time.time()
        #         now = time.time()
...
if __name__ == '__main__':
    ...
    # Pink Mark
    p_min_blue, p_min_green, p_min_red = 44, 0, 255
    p_max_blue, p_max_green, p_max_red = 110, 255, 255
    pta, pba = 650, 30000
